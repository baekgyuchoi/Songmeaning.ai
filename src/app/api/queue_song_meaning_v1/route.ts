...
export async function POST(req: Request) {
    ...
    try {
        ...
        const gptResponse = await openai.chat.completions.create({
            messages: [{ role: 'user', content: v1songMeaningPrompt + songMeaningContext}],
        ...
        console.log(song_meaning_v2)

        if (gptResponse.choices[0].message.content!.length > 400) {
            //format meaning
            //save each section separately

            const meaning = gptResponse.choices[0].message.content
            ...
            formatted_meaning.meaning = song_meaning_v2!
            ...
            console.error("explicit content? content cutoff error")
        }
